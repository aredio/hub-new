...
class AssetAllocationCalculator {
    ...
    updateProfileInfo(profile) {
        ...
        if (profileName)
            profileName.textContent = profile.type;
    ...
    createAllocationSliders(allocation) {
    ...
    resetToSuggested() {
        ...
        this.currentAllocation.forEach((item, index) => {
            ...
            const valueElement = document.getElementById(`sliderValue${index}`);
            if (slider)
                slider.value = item.pct.toString();
            if (valueElement)
                valueElement.textContent = `${item.pct}%`;
        });
        this.updateChart();
        this.updateTotalValidation();
    }
    ...
    generateCSV(results) {
        ...
        const rows = [
            ...
            ...results.allocation.map((item) => ['', '', item.class, item.pct, item.description])
        ];
...
//# sourceMappingURL=alocacao.js.map