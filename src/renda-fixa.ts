...
interface RendaFixaInput {
    valor: number;
    prazo: number;
    prazoTipo: 'dias' | 'meses' | 'anos';
    taxa: number;
    taxaTipo: 'aa' | 'am';
    tipoAplicacao: 'CDB' | 'LCI' | 'LCA' | 'Tesouro';
    compounding: 'daily' | 'monthly' | 'yearly';
    inflacao: number;
    incidenciaIR: boolean;
    incidenciaIOF: boolean;
...
class RendaFixaSimulator {
    ...
    private currentResult: RendaFixaResult | null = null;
    ...
    private initializeEventListeners(): void {
        const form = document.getElementById('rendaFixaForm') as HTMLFormElement;
    ...
    private setupFormValidation(): void {
        ...
        inputs.forEach(input => {
            input.addEventListener('blur', () => this.validateInput(input as HTMLInputElement | HTMLSelectElement));
        });
    }

    private validateInput(input: HTMLInputElement | HTMLSelectElement): void {
    ...
    private showInputError(input: HTMLInputElement | HTMLSelectElement, message: string): void {
        this.clearError(input);
    ...
    private clearError(input: HTMLInputElement | HTMLSelectElement): void {
    ...
    private getFormData(): RendaFixaInput {
        const form = document.getElementById('rendaFixaForm') as HTMLFormElement;
        const formData = new FormData(form);

        return {
            valor: parseFloat(formData.get('valor') as string) || 0,
            prazo: parseInt(formData.get('prazo') as string) || 0,
            prazoTipo: formData.get('prazoTipo') as 'dias' | 'meses' | 'anos',
            taxa: parseFloat(formData.get('taxa') as string) || 0,
            taxaTipo: formData.get('taxaTipo') as 'aa' | 'am',
            tipoAplicacao: formData.get('tipoAplicacao') as 'CDB' | 'LCI' | 'LCA' | 'Tesouro',
            compounding: formData.get('compounding') as 'daily' | 'monthly' | 'yearly',
            inflacao: parseFloat(formData.get('inflacao') as string) || 0,
    ...
    private validateFormData(data: RendaFixaInput): boolean {
        return data.valor > 0 && data.prazo > 0 && data.taxa >= 0 && data.inflacao >= 0;
    }

    private simulate(input: RendaFixaInput): void {
        try {
            this.showLoading();
    ...
    private converterPrazoParaDias(prazo: number, tipo: string): number {
        switch (tipo) {
    ...
    private converterTaxaParaDiaria(taxa: number, tipo: string): number {
        switch (tipo) {
    ...
    private calcularSchedule(input: RendaFixaInput, prazoEmDias: number, taxaDiaria: number): FluxoItem[] {
        const schedule: FluxoItem[] = [];
    ...
    private getFrequenciaCapitalizacao(compounding: string): number {
        switch (compounding) {
            case 'daily': return 1;
    ...
    private calcularSaldoBruto(valorInicial: number, taxaDiaria: number, dias: number, compounding: string): number {
        switch (compounding) {
            case 'daily':
    ...
    private calcularIR(saldoBruto: number, valorInicial: number, dias: number, input: RendaFixaInput): number {
        if (!input.incidenciaIR) return 0;
        
        // LCI/LCA são isentas de IR
        if (input.tipoAplicacao === 'LCI' || input.tipoAplicacao === 'LCA') return 0;
    ...
    private calcularIOF(saldoBruto: number, valorInicial: number, dias: number, input: RendaFixaInput): number {
        if (!input.incidenciaIOF || dias > 30) return 0;
    ...
    private getAliquotaIOF(dias: number): number {
        if (dias <= 1) return 0.96;   // 96%
        if (dias <= 2) return 0.93;   // 93%
        if (dias <= 3) return 0.90;   // 90%
        if (dias <= 4) return 0.86;   // 86%
        if (dias <= 5) return 0.83;   // 83%
        if (dias <= 6) return 0.80;   // 80%
        if (dias <= 7) return 0.76;   // 76%
        if (dias <= 8) return 0.73;   // 73%
        if (dias <= 9) return 0.70;   // 70%
        if (dias <= 10) return 0.66;  // 66%
        if (dias <= 11) return 0.63;  // 63%
        if (dias <= 12) return 0.60;  // 60%
        if (dias <= 13) return 0.56;  // 56%
        if (dias <= 14) return 0.53;  // 53%
        if (dias <= 15) return 0.50;  // 50%
        if (dias <= 16) return 0.46;  // 46%
        if (dias <= 17) return 0.43;  // 43%
        if (dias <= 18) return 0.40;  // 40%
        if (dias <= 19) return 0.36;  // 36%
        if (dias <= 20) return 0.33;  // 33%
        if (dias <= 21) return 0.30;  // 30%
        if (dias <= 22) return 0.26;  // 26%
        if (dias <= 23) return 0.23;  // 23%
        if (dias <= 24) return 0.20;  // 20%
        if (dias <= 25) return 0.16;  // 16%
        if (dias <= 26) return 0.13;  // 13%
        if (dias <= 27) return 0.10;  // 10%
        if (dias <= 28) return 0.06;  // 6%
        if (dias <= 29) return 0.03;  // 3%
        return 0; // 0% após 30 dias
    }

    private calcularTotais(input: RendaFixaInput, schedule: FluxoItem[]): any {
    ...
    private displayResults(result: RendaFixaResult): void {
    ...
    private updateTable(schedule: FluxoItem[]): void {
        const tbody = document.getElementById('fluxoTableBody') as HTMLElement;
        tbody.innerHTML = '';
        ...
        schedule.forEach((item, index) => {
            if (index % step === 0 || index === schedule.length - 1) {
                const row = (tbody as HTMLTableSectionElement).insertRow();
                row.innerHTML = `
                    <td>${item.data}</td>
    ...
    private createChart(result: RendaFixaResult): void {
    ...
    private generateCSV(result: RendaFixaResult): string {
    ...
    private generatePDF(result: RendaFixaResult): string {
    ...
    private showLoading(): void {
        const button = document.querySelector('.simulate-button') as HTMLButtonElement;
